...
def load_data():
    ...
    # ✅ FIXED: robust timestamp parsing — format='mixed' handles both space and ISO T formats
    price["timestamp"] = pd.to_datetime(price["timestamp"], format="mixed", utc=True, errors="coerce")
    decisions["timestamp"] = pd.to_datetime(decisions["timestamp"], format="mixed", utc=True, errors="coerce")

    # ✅ drop bad rows instead of crashing
    price = price.dropna(subset=["timestamp"])
    decisions = decisions.dropna(subset=["timestamp"])
...
def main():
    ...
    if not results:
        too_recent = sum(
            1 for _, row in decisions.iterrows()
            if row["timestamp"] not in evaluated_timestamps
            and len(price[
                (price["timestamp"] > row["timestamp"]) &
                (price["timestamp"] <= row["timestamp"] + pd.Timedelta(hours=EVAL_WINDOW_HOURS))
            ]) < EVAL_WINDOW_HOURS
        )
        eligible_count = (
            len(decisions)
            - len(evaluated_timestamps)
            - too_recent
        )
        print(
            f"No new evaluation rows. "
            f"decisions={len(decisions)} "
            f"already_evaluated={len(evaluated_timestamps)} "
            f"too_recent={too_recent} "
            f"eligible_but_skipped={eligible_count}"
        )
        return
